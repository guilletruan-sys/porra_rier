  ...
  return (
    <div className="bg-gradient-to-br from-red-600 to-red-700 rounded-xl shadow-md p-3 text-white relative overflow-hidden">
      <div className="absolute top-2 right-2 flex items-center gap-1 text-[10px] font-bold uppercase tracking-wider">
        <span className="inline-block w-1.5 h-1.5 rounded-full bg-white dark:bg-slate-900 animate-pulse" />
        {minuteLabel}
      </div>
      ...
      <div className="flex items-center justify-between gap-3 mt-1">
        ...
        {/* Score — clickable, abre el sheet con stats, cuotas y predicciones de la porra */}
        <Spoiler>
          <button
            type="button"
            onClick={() => setOpen(true)}
            aria-label="Ver detalles del partido"
            className="flex items-center gap-2 shrink-0 px-3 py-1.5 bg-white/15 dark:bg-slate-800/15 hover:bg-white/25 dark:hover:bg-slate-800/25 backdrop-blur-sm rounded-lg transition-colors active:scale-95"
          >
      ...
      <a
        ...
        className="mt-3 flex items-center justify-center gap-1.5 bg-white/15 dark:bg-slate-800/15 hover:bg-white/25 dark:hover:bg-slate-800/25 backdrop-blur-sm rounded-lg py-1.5 text-[10px] font-bold transition-colors"
      >
