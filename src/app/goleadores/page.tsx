...
export default function GoleadoresPage() {
  ...
  return (
    <div className="p-3">
      ...
      {loading && (
        <div className="text-center text-sm text-slate-400 py-8">Cargando goleadores…</div>
      )}
      ...
      {scorers.length > 0 && (
        ...
        <div className="bg-white rounded-xl shadow-sm overflow-hidden">
          <table className="w-full">
            <thead>
              <tr className="text-[8px] font-bold text-slate-400 uppercase border-b border-slate-50">
            ...
            <tbody>
              {scorers.map((s, i) => {
                ...
                const isTop = i === 0
                return (
                  <tr key={s.player.id} className={`border-t border-slate-50 ${isTop ? 'bg-amber-50/60' : ''}`}>
                    <td className="pl-3 py-2.5">
                      <RankBadge rank={i + 1} />
                    </td>
                    <td className="py-2.5 pr-2">
                      <Link href={`/jugador/${s.player.id}`} className="block hover:opacity-80">
                        <p className="text-[10px] font-bold text-slate-800 truncate underline decoration-dotted underline-offset-2">{s.player.name}</p>
                        <div className="flex items-center gap-1 mt-0.5">
                          ...
                          <span className="text-[9px] text-slate-400">{s.team.shortName}</span>
                        </div>
                      </Link>
                    </td>
                    <td className="py-2.5 text-center text-[10px] text-slate-500">{s.playedMatches}</td>
                    <td className="py-2.5 text-center">
                      <span className={`text-sm font-black ${isTop ? 'text-[#c8102e]' : 'text-slate-700'}`}>
                        {s.goals}
                      </span>
                    </td>
                    <td className="pr-3 py-2.5 text-center text-[10px] text-slate-500">
                      {s.assists ?? 0}
                    </td>
                  </tr>
                )
